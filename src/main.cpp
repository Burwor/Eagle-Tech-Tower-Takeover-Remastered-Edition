...
void open_stack()
{
	...
	intake(0);
	pros::delay(700);
	arm_mtr1.move_relative(1000, 127);
	arm_mtr2.move_relative(1000, 127);
...
void autonomous()
{
	...
	open_stack();
	autonomous_recorder = fopen("/usd/autonomous_recorder1.txt", "r");
	do
	{
...
void opcontrol()
{
	...
	while (true)
	{
		...
		{
			autonomous_recorder = fopen("/usd/autonomous_recorder1.txt", "w");
			open_stack();
